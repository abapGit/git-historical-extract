...
- [x] Read the historical version into `zif_abapgit_object_tabl=>ty_internal`, filling the components the DDL serializer actually consumes: `DD02V`, `DD03P`, `DD05M`, `DD08V`, `DD35V`, and `DD36M`.
- [x] Merge the original-language table texts returned by the version reader into `DD02V-DDTEXT` the way `read_domd` does for domains, because both the DDL `@EndUserText.label` annotation and the AFF header description are taken from that one field.
