CLASS zcl_abapgit_historical_prog DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_ABAPGIT_HISTORICAL_PROG IMPLEMENTATION.
  ...
  METHOD zif_abapgit_historical_object~build_files.
    ...
    ls_file-filename = |{ to_lower( ms_tadir-obj_name ) }.prog.abap|.

    READ TABLE lt_vrsd INTO DATA(ls_extended) WITH KEY objtype = 'REPS'.
    IF sy-subrc = 0.
  ...
  METHOD zif_abapgit_historical_object~build_deleted_files.
    ...
    APPEND VALUE #(
      filename = |{ to_lower( ms_tadir-obj_name ) }.prog.abap|
